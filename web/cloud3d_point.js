...
function updatePoints(points, particleSystem) {
  var n = points.length;
  var p = particleSystem.geometry.vertices;
  for (var i = 0; i < n; i++) {
    curPoint = points[i];
    // 判断粒子是否死亡
    // 越高 寿命减少的越快
    ...
    if (curPoint.remain_life <= 0) {
      ...
      // 计算粒子运动
      curPoint.position.x += curPoint.speed.x;
      curPoint.position.y += curPoint.speed.y;
      curPoint.position.z += curPoint.speed.z;
    }
    p[i].x = curPoint.position.x;
    p[i].y = curPoint.position.y;
    p[i].z = curPoint.position.z;
  }
}
