    ...
    for (var i = 0; i < n; i++) {
        curPoint = {
            ...
            speed: {
                ...
                // 粒子上升速度的随机干扰由1维正向云生成
                // u = 0.5, sigma = 0.1
                y: Math.sqrt(cloud2dModel[i][2] * Math.abs(cloud1dModel[i][0]))
                // y: cloud2dModel[i][2] * normrnd(0.5, 0.1)
            },
            // 寿命
            // 寿命的随机干扰由1维正向云生成
            // remain_life: 150 * normrnd(5, 5)
            remain_life: 750 * cloud1dLifeModel[i][0]
        };
...
function updatePoints(points, particleSystem) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        if (curPoint.remain_life <= 0) {
            // 如果死亡 生成新粒子
            points[i] = generatePoints(1, window.cloudModelConfig)[0];
        }
        else {
            // 计算粒子运动
